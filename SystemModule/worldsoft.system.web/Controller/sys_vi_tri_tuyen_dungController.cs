...
{
    public partial class sys_vi_tri_tuyen_dungController : BaseAuthenticationController
    {
        public sys_vi_tri_tuyen_dung_repo repo;

        public sys_vi_tri_tuyen_dungController(IUserService userService, worldsoftDefautContext context) : base(userService)
        {
            repo = new sys_vi_tri_tuyen_dung_repo(context);
        }

        public IActionResult getListUse()
        {
            var result = repo._context.sys_vi_tri_tuyen_dungs
        ...
        public async Task<IActionResult> create([FromBody] JObject json)
        {
            ...
            var model = JsonConvert.DeserializeObject<sys_vi_tri_tuyen_dung_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                var dataList = await Task.Run(() => query.OrderByDescending(d => d.db.update_date).Skip(param.Start).Take(param.Length)
                   .ToList());
                DTResult<sys_vi_tri_tuyen_dung_model> result = new DTResult<sys_vi_tri_tuyen_dung_model>
                {
                    start = param.Start,
                    draw = param.Draw,
                    data = dataList,
