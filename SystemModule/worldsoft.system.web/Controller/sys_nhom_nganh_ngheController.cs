...
{
    public partial class sys_nhom_nganh_ngheController : BaseAuthenticationController
    {
        public sys_nhom_nganh_nghe_repo repo;

        public sys_nhom_nganh_ngheController(IUserService userService, worldsoftDefautContext context) : base(userService)
        {
            repo = new sys_nhom_nganh_nghe_repo(context);
        }

        public IActionResult getListUse()
        {
            var result = repo._context.sys_nhom_nganh_nghes
        ...
        public async Task<IActionResult> create([FromBody] JObject json)
        {
            ...
            var model = JsonConvert.DeserializeObject<sys_nhom_nganh_nghe_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                var dataList = await Task.Run(() => query.OrderByDescending(d => d.db.update_date).Skip(param.Start).Take(param.Length)
                   .ToList());
                DTResult<sys_nhom_nganh_nghe_model> result = new DTResult<sys_nhom_nganh_nghe_model>
                {
                    start = param.Start,
                    draw = param.Draw,
                    data = dataList,
