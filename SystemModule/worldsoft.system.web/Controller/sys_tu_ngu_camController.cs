...
{
    public partial class sys_tu_ngu_camController : BaseAuthenticationController
    {
        public sys_tu_ngu_cam_repo repo;

        public sys_tu_ngu_camController(IUserService userService, worldsoftDefautContext context) : base(userService)
        {
            repo = new sys_tu_ngu_cam_repo(context);
        }

        public IActionResult getListUse()
        {
            var result = repo._context.sys_tu_ngu_cams
        ...
        public async Task<IActionResult> create([FromBody] JObject json)
        {
            ...
            var model = JsonConvert.DeserializeObject<sys_tu_ngu_cam_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                var dataList = await Task.Run(() => query.OrderByDescending(d => d.db.update_date).Skip(param.Start).Take(param.Length)
                   .ToList());
                DTResult<sys_tu_ngu_cam_model> result = new DTResult<sys_tu_ngu_cam_model>
                {
                    start = param.Start,
                    draw = param.Draw,
                    data = dataList,
