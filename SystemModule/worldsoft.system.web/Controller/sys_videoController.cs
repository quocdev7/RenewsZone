...
{
    public partial class sys_videoController : BaseAuthenticationController
    {
        private sys_video_repo repo;

        public sys_videoController(IUserService userService,worldsoftDefautContext context) : base(userService)
        {
            repo = new sys_video_repo(context);
        }

        public IActionResult getListUse()
        {
            var result = repo._context.sys_videos
                .Where(d=>d.status_del==1)
                 .Select(d => new
                 {
                     ...
                     name = d.name,
                     name_en = d.name_en,
                     image_thumnail = d.image_thumnail
                 }).ToList();
            return Json(result);
        }
        [HttpPost]
        public async Task<IActionResult> create([FromBody] JObject json)
        {
            ...
            var model = JsonConvert.DeserializeObject<sys_video_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
          ...
          .ToList());
                DTResult<sys_video_model> result = new DTResult<sys_video_model>
                {
                    start = param.Start,
                    draw = param.Draw,
                    data = dataList,
