...
{
    public partial class sys_anh_san_phamController : BaseAuthenticationController
    {
        private sys_anh_san_pham_repo repo;
        public AppSettings _appsetting;
        public sys_anh_san_phamController(IUserService userService,worldsoftDefautContext context, IOptions<AppSettings> appsetting) : base(userService)
        {
            _appsetting = appsetting.Value;
            repo = new sys_anh_san_pham_repo(context);
        }
        ...
        //    if (ids.Count() == 1)
        //    {
        //        var filedb = repo._context.sys_san_pham_file_uploads.Where(t => t.id == id).FirstOrDefault();



        //        var path = Path.Combine(_appsetting.folder_path, "file_upload", "image_sp", filedb.id) + filedb.file_path;
        ...
        //        {
        //            var filedb = repo._context.sys_san_pham_file_uploads.Where(t => t.id == ids[i]).FirstOrDefault();
        //            var path = _appsetting.folder_path + filedb.file_path;
        //            var model = new ZipItem();
        //            model.file_name = filedb.file_name;
        //            model.file_path = path;
        //            model.name = repo._context.sys_san_phams.Where(t => t.id == filedb.id_san_pham).Select(q => q.ten_san_pham).FirstOrDefault();
        ...
        //        { FileDownloadName = zipItems[0].name + "." + "zip" };
        ...
        public IActionResult getListUse()
        {
            ...
            var result = repo._context.sys_anh_san_phams
                ...
                .Where(d=>d.status_del==1)
                 .Select(d => new
                 {
                     id = d.id,
                     image_web=d.image,
                     image_mobile = d.image_mobile,
                     name = repo._context.sys_san_phams.Where(t => t.id == d.id_san_pham).Select(d => d.ten_san_pham).SingleOrDefault(),
                 }).ToList();
            return Json(result);
        }
        [HttpPost]
        public async Task<IActionResult> create([FromBody] JObject json)
        {
            ...
            var model = JsonConvert.DeserializeObject<sys_anh_san_pham_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                var id_san_pham = dictionary["id_san_pham"];
                var search = dictionary["search"];
                var query = repo.FindAll()
                        //.Where(d => d.db.status_del == 1)
                     .Where(d => d.db.name.Contains(search))
                       .Where(d => d.db.id_san_pham.Equals(id_san_pham) || id_san_pham == "-1")
                     ;
          ...
          .ToList());
                DTResult<sys_anh_san_pham_model> result = new DTResult<sys_anh_san_pham_model>
                {
                    start = param.Start,
                    draw = param.Draw,
                    data = dataList,
