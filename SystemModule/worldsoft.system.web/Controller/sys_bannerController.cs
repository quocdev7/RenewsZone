...
namespace worldsoft.system.web.Controller
{
    public partial class sys_bannerController : BaseAuthenticationController
    {
        ...
        public sys_bannerController(IUserService userService, worldsoftDefautContext context) : base(userService)
        {
        ...
        {
            var result = repo._context.sys_banners
                .OrderBy(d => d.stt)
                .Where(d => d.status_del == 1 && d.loai == 1)
                 .Select(d => new sys_banner_model
                 {
                     db =d
                 }).ToList();
            return Json(result);
        }
        public IActionResult getListUseContactUs()
        {
            var result = repo._context.sys_banners
                .OrderBy(d => d.stt)
                .Where(d => d.status_del == 1 && d.loai == 2)
                 .Select(d => new sys_banner_model
                 {
                     db = d
                 }).ToList();
