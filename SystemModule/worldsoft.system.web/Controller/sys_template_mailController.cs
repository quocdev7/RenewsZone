...
namespace worldsoft.system.web.Controller
{
    public partial class sys_template_mailController : BaseAuthenticationController
    {
        ...
        public sys_template_mailController(IUserService userService,worldsoftDefautContext context) : base(userService)
        {
        ...
        {
            var result = repo._context.sys_template_mails
               .Where(d => d.status_del == 1).
                Select(d => new
                {
                    ...
                    tieu_de = d.name,
                    noi_dung = d.template,
                    type_code = repo._context.sys_type_mails.Where(q=>q.id == d.id_type).Select(q=>q.code).FirstOrDefault()
                }).ToList();
            return Json(result);
        }
