...
{
    public partial class sys_nghi_quyetController : BaseAuthenticationController
    {
        public sys_nghi_quyet_repo repo;

        public sys_nghi_quyetController(IUserService userService, worldsoftDefautContext context) : base(userService)
        {
            repo = new sys_nghi_quyet_repo(context);
        }

        public IActionResult getListUse()
        {
            var result = repo._context.sys_nghi_quyets
        ...
        public async Task<IActionResult> create([FromBody] JObject json)
        {
            ...
            var model = JsonConvert.DeserializeObject<sys_nghi_quyet_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                var dataList = await Task.Run(() => query.OrderByDescending(d => d.db.update_date).Skip(param.Start).Take(param.Length)
                   .ToList());
                DTResult<sys_nghi_quyet_model> result = new DTResult<sys_nghi_quyet_model>
                {
                    start = param.Start,
                    draw = param.Draw,
                    data = dataList,
