...
{
    public partial class sys_khu_vucController : BaseAuthenticationController
    {
        public sys_khu_vuc_repo repo;

        public sys_khu_vucController(IUserService userService, worldsoftDefautContext context) : base(userService)
        {
            repo = new sys_khu_vuc_repo(context);
        }

        public IActionResult getListUse()
        {
            var result = repo._context.sys_khu_vucs
        ...
        public async Task<IActionResult> create([FromBody] JObject json)
        {
            ...
            var model = JsonConvert.DeserializeObject<sys_khu_vuc_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                var dataList = await Task.Run(() => query.OrderByDescending(d => d.db.update_date).Skip(param.Start).Take(param.Length)
                   .ToList());
                DTResult<sys_khu_vuc_model> result = new DTResult<sys_khu_vuc_model>
                {
                    start = param.Start,
                    draw = param.Draw,
                    data = dataList,
