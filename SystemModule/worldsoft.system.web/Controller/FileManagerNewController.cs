...
namespace worldsoft.common.Controllers
{

    [ApiController]
    [Route("[controller]")]
    public class FileManagerNewController : Controller
    {
        public AppSettings _appsetting;
        public FileManagerNewController( IOptions<AppSettings> appsetting) 
        {
        ...
        public async Task<ActionResult> Download(string filename, string name)
        {
            ...
            var current = _appsetting.folder_path;  //Directory.GetCurrentDirectory();
        ...
        public async Task<ActionResult> uploadimagenew()
        {
            ...
            if (item != null)
            {
                ...
                var path = Path.Combine(currentpath, "file_upload", "image_upload", controller,user_id, type);
                if (!Directory.Exists(path))
                    Directory.CreateDirectory(path);
        ...
        [HttpPost("upload_chat")]
        public async Task<ActionResult> upload_chat()
        {
            //file = JsonConvert.DeserializeObject<sys_file_model>(Request.Form["thumbnail"]);
            var request = Request;
            var user_id = User.Identity.Name ?? "nonlogin";
