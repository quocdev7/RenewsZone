...
{
    public partial class sys_khoaController : BaseAuthenticationController
    {
        public sys_khoa_repo repo;

        public sys_khoaController(IUserService userService, worldsoftDefautContext context) : base(userService)
        {
            repo = new sys_khoa_repo(context);
        }

        public IActionResult getListUse()
        {
            var result = repo._context.sys_khoas
        ...
        public async Task<IActionResult> create([FromBody] JObject json)
        {
            ...
            var model = JsonConvert.DeserializeObject<sys_khoa_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                var dataList = await Task.Run(() => query.OrderByDescending(d => d.db.stt).Skip(param.Start).Take(param.Length)
                   .ToList());
                DTResult<sys_khoa_model> result = new DTResult<sys_khoa_model>
                {
                    start = param.Start,
                    draw = param.Draw,
                    data = dataList,
