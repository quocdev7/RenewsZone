...
{
    public class sys_event_repo
    {
        public worldsoftDefautContext _context;

        public sys_event_repo(worldsoftDefautContext context)
        {
            _context = context;
        }

        public async Task<sys_event_model> getElementById(string id)
        {
        ...
        public async Task<int> insert(sys_event_model model)
        {
        ...
        public async Task<int> insert_en(sys_event_en_model model_language)
        {
            await _context.sys_event_ens.AddAsync(model_language.db);
            _context.SaveChanges();
            return 1;
        }
        public async Task<int> update(sys_event_model model)
        {
            ...
            db.luu_y_tham_gia_mobile = model.db.luu_y_tham_gia_mobile;
            _context.SaveChanges();
            insert_search(db);
            return 1;
        }
        ...
        public IQueryable<sys_event_model> FindAll()
        {
            var result = _context.sys_events.Select(d => new sys_event_model()
            {
        ...
        public IQueryable<sys_event_model> FindAllNewPortal(string user_id)
        {
            var result = _context.sys_events.Select(d => new sys_event_model()
            {
                ten_khoa = _context.sys_khoas.Where(t => t.id == d.id_khoa).Select(d => d.name).SingleOrDefault(),
