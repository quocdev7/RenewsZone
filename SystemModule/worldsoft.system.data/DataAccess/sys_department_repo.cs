...
namespace worldsoft.system.data.DataAccess
{
    public class sys_department_repo
    {
        ...
        public sys_department_repo(worldsoftDefautContext context)
        {
            _context = context;
        }
        ...
        public async Task<int> update(sys_department_model model)
        {
            var db = await _context.sys_departments.Where(d => d.id == model.db.id).FirstOrDefaultAsync();
