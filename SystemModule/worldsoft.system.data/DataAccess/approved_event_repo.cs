...
{
    public class approved_event_repo 
    {
        public worldsoftDefautContext _context;

        public approved_event_repo(worldsoftDefautContext context)
        {
            _context = context;
        }

        public async Task<approved_event_model> getElementById(string id)
        {
        ...
        public async Task<int> insert(approved_event_model model)
        {
        ...
        public async Task<int> update(approved_event_model model)
        {
        ...
        public IQueryable<approved_event_model> FindAll()
        {
            var result = _context.sys_events.Select(d=> new approved_event_model()
            {
        ...
        public IQueryable<approved_event_model> FindAllNewPortal(string user_id)
        {
            var result = _context.sys_events.Select(d => new approved_event_model()
            {
                ten_khoa = _context.sys_khoas.Where(t => t.id == d.id_khoa).Select(d => d.name).SingleOrDefault(),
