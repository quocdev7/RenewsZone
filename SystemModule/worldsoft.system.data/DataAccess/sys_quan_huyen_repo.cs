...
namespace worldsoft.system.data.DataAccess
{
    public class sys_quan_huyen_repo
    {
        ...
        public sys_quan_huyen_repo(worldsoftDefautContext context)
        {
            _context = context;
        }

        public async Task<sys_quan_huyen_model> getElementById(long id)
        {
        ...
        {
            var db = await _context.sys_quan_huyens.Where(d => d.id == model.db.id).FirstOrDefaultAsync();
        ...
        {
            var result = _context.sys_quan_huyens.Select(d => new sys_quan_huyen_model()
            {
                ...
                tinh = _context.sys_tinh_thanhs.Where(t => t.id == d.id_tinh).Select(d => d.ten).SingleOrDefault(),
                db = d,
            });
        ...
        {
            var itemToRemove = _context.sys_quan_huyens.Where(x => x.id == id).FirstOrDefault();
        ...
        {
            var itemToRemove = _context.sys_quan_huyens.Where(x => x.id == long.Parse(id)).FirstOrDefault();
